...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    """Save images to the disk. In case of 3D input the data is saved as numpy arrays (.npy)  
    ...
    for label, im_data in visuals.items():
        ...
        if len(im_data.shape) == 4:
            ...
            if use_wandb:
                wandb.log(ims_dict)
        # save 3D volumes ass numpy .npy 
        elif len(im_data.shape) == 5:
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, save_result):
                ...
                try:
                    for label, image in visuals.items():
                        ...
                        # for volume data
                            ...
                            x = np.linspace(0 , self.opt.vs_x-1, image_numpy.shape[1])
                            y = np.linspace(0 , self.opt.vs_y-1, image_numpy.shape[2])
                            z = np.linspace(0 , self.opt.vs_z-1, image_numpy.shape[0])
                            xx, yy, zz = np.meshgrid(x, z, y, indexing='xy')
                            xyz = np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T.astype(np.uint8)
                            c = np.reshape(image_numpy, image_numpy.shape[0] * image_numpy.shape[1] * image_numpy.shape[2])

                            self.vis.scatter(X=xyz, opts=dict(title=label, markercolor=np.vstack([c,c,c]).T, markersize=3, markerborderwidth=2),
                                        win=self.display_id + idx)
                        idx += 1
                except VisdomExceptionBase:
                    self.create_visdom_connections()
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            # update website
            webpage = single_html.HTML(self.web_dir, 'Experiment name = %s' % self.name, refresh=1)
